'EBA Command Center 9 | Windows XP Edition
'Version 9.12-beta1-WinXP (0920002-XP)
'Update 10/19/21
...
'
'In compliance with the EBA Command Center EULA, modification developers are required to make notice that this is a modification. If you do not see this notice, this is an official copy of EBA Command Center.
Option Explicit
On Error Resume Next
Dim Edition : Edition = "WindowsXP"
Dim ver : ver = "Release 1.2-beta1"
Dim verID : verID = 9.1

'Objects
Dim app : Set app = CreateObject("Shell.Application")
Dim backup1, backup2, forVar, forVar1, forVar2, forVar3, forVar4, short, sys
...
lang(014) = "You can edit your settings using the Config menu! To access this menu, you'll need to run the command 'config'. You might need to enter your username and password first. Advanced settings are found in the registry, at \r."
...
lang(042) = "Guest"
lang(043) = "Type 'help' for a list of commands."
lang(044) = "{1}\l{2}"
...
lang(411) = "Debug Mode"
lang(412) = "Maximum Plugings are too high! The maximum value is 40. We've updated that for you."
...
Sub modeFirstRun
	...
	If eba = vbYes Then
		eba = Replace(inputbox(strParam1(lang(005),lang(006)),title),"""","")
	...
	newFolder(dataLoc & "\Plugins")
	newFolder(dataLoc & "\PluginData")
	updateSet True, False, "shutdown", 10, 2
	updateInf Edition, "", 0, ver, verID, language
	If uName <> "" Then
...
Sub modeNormal
	...
	If Not skipDo Then
		...
		If Not saveLogin Then updateInf Edition, user, userPerms, ver, verID, language
	...
	Do
		...
		Elseif exeValue = "eba.backup" Then
			eba = Replace(Replace(LCase(inputbox(strParam1(lang(005),lang(006)),title,"""" & dataLoc & "\backup.ebabackup""")),".ebabackup",""),"""","") & ".ebabackup"
		...
		Elseif exeValue = "eba.config" Then
			...
			If eba = "cmd" or eba = "command" Then
				...
				If fExists(dataLoc & "\Cmd\" & eba & ".ebacmd") Then
					...
					If param(3) = "" Then param(3) = inputbox(strParam1(lang(029),strParam2(lang(022),"Target",lang(064)) & strParam2(lang(028),"Login",lang(065)) & strParam2(lang(028),"Admin",lang(066)) & strParam2(lang(028),"Delete",lang(067))),title)
					eba = LCase(param(3))
					
					If eba = "target" Then
						temp(2) = "target"
						temp(3) = LCase(inputbox(strParam1(lang(005),lang(70)),title,temp(5)))
						temp(4) = True
						temp(8) = temp(5)
						temp(5) = temp(3)
					Elseif eba = "name" Then
						temp(2) = "name"
						temp(3) = LCase(inputbox(strParam1(lang(005),lang(071)),title,temp(1)))
						temp(4) = True
						temp(8) = temp(1)
				...
				Else
					...
					Else
						...
						Elseif eba = "import" Then
							temp(1) = "cmd"
							temp(2) = Replace(LCase(inputbox(strParam1(lang(005),lang(70)),title)),"""","")
							If fExists(temp(2)) Then
			...
			Elseif eba = "acc" or eba = "account" Then
				...
				If fExists(dataLoc & "\User\" & eba & ".ebacmd") Then
					...
					temp(0) = eba
					If param(3) = "" Then param(3) = inputbox(strParam1(lang(029),strParam2(lang(022),"Pword",lang(012)) & strParam2(lang(028),"Perms",lang(094)) & strParam2(lang(028),"Delete",lang(095))),title)
					eba = LCase(param(3))
					If eba = "pword" Then
			...
			Elseif eba = "logs" Then
				...
				eba = LCase(param(2))
				If eba = "false" Then
					updateSet False, saveLogin, defaultshutdown, shutdowntimer, blockMalware
					Call readSettings
				Elseif eba = "true" Then
					updateSet True, saveLogin, defaultShutdown, shutdownTimer, blockMalware
					Call readSettings
				Else
					Warn lang(516)
				End If
			Elseif eba = "save" or eba = "savelogin" Then
				...
				eba = LCase(param(2))
				If eba = "false" Then
					updateSet logging, False, defaultshutdown, shutdowntimer, blockMalware
					Call readSettings
				Elseif eba = "true" Then
					updateSet logging, True, defaultShutdown, shutdownTimer, blockMalware
					Call readSettings
				Else
					Warn lang(516)
				End If
			Elseif eba = "time" or eba = "shutdowntimer" Then
				...
				If CInt(eba) >= 0 Then
					If Err.Number = 0 Then
						updateSet logging, saveLogin, defaultShutdown, eba, blockMalware
						Call readSettings
					End If
				Else
					Warn lang(516)
			...
			Elseif eba = "shut" or eba = "defaultshutdown" Then
				...
				Elseif eba = "shutdown" or eba = "s" Then
					updateSet logging, saveLogin, "shutdown", shutdownTimer, blockMalware
				Elseif eba = "restart" or eba = "r" Then
					updateSet logging, saveLogin, "restart", shutdownTimer, blockMalware
				Elseif eba = "hibernate" or eba = "h" Then
					updateSet logging, saveLogin, "hibernate", shutdownTimer, blockMalware
				Else
					Warn lang(516)
				End If
				Call readSettings
			...
			Elseif eba = "mal" or eba = "malware" Then
				...
				Elseif CInt(eba) >= 0 And CInt(eba) <= 3 Then
					If Err.Number = 0 Then
						updateSet logging, saveLogin, defaultShutdown, shutdownTimer, eba
					End If
				Else
					Warn lang(516)
				End If
		...
		Elseif exeValue = "eba.import" Then
			importData = Replace(inputbox(strParam1(lang(005),lang(088)),title),"""","")
		...
		Elseif exeValue = "eba.login" Then
			...
			If fExists(dataLoc & "\User\" & uName & ".ebacmd") Then
				...
				If lines(1) = "" Then
					updateInf Edition, uName, getNode(dataLoc & "\User\" & uName & ".ebacmd","User/Perms"), ver, verID, language
		...
		Elseif exeValue = "eba.logout" Then
			updateInf Edition, "InvalidUser", 0, ver, verID, language
...
Sub modeRefresh
	...
	End If
	Do
		uName = inputbox("Please enter your username:",title)
		If fExists(dataLoc & "\User\" & uName & ".ebacmd") Then
			If getNode(dataLoc & "\User\" & uName & ".ebacmd","User/Perms") = "3" Then
				pWord = inputbox("Please enter your password:",title)
				If pWord = LCase(decrypt(read(dataLoc & "\User\" & uName & "-enc.ebacmd","n"))) Then
					Exit Do
				Else
					Error "Incorrect password."
				End If
			Else
				Error "That account does not have a high enough permission level. You need permission level 3."
			End If
		Elseif uName = "" Then
			Call endOp("r")
		Else
...
Sub modeUninstall
	...
	Do
		...
		If fExists(dataLoc & "\User\" & uName & ".ebacmd") Then
			If getNode(dataLoc & "\User\" & uName & ".ebacmd","User/Perms") = "3" Then
				pWord = inputbox(strParam1(lang(009),lang(012)),title)
				If pWord = decrypt(read(dataLoc & "\User\" & uName & "-enc.ebacmd","n")) Then
					Exit Do
				Else
					Error lang(142), lang(217)
				End If
			Else
				Warn strParam2(lang(511),3,getNode(dataLoc & "\User\" & uName & ".ebacmd","User/Perms"))
			End If
		Elseif uName = "" Then
			write dataLoc & "\startupType.ebacmd", "normal"
			Call endOp("r")
...
Sub checkBlacklist
	If Edition = "Windows10" Then
		...
		read dataLocTmp & "\blocked.ebacmd","l"
		...
		If InStr(data,"<name>" & LCase(pcName) & "</name>") Then
			...
			Critical lang(122), strParam1(lang(211),pcName)
			Call endOp("s")
		Elseif InStr(data,"<user>" & LCase(tmp) & "</user>") Then
			log lang(302) & strParam1(lang(311),strParam1(lang(212),tmp))
			Critical lang(122), strParam1(lang(212),tmp)
			Call endOp("s")
		Elseif InStr(data,"<domain>" & LCase(domain) & "</domain>") Then
			log lang(302) & strParam1(lang(311),strParam1(lang(214),domain))
			Critical lang(122), strParam1(lang(214),domain)
			Call endOp("s")
		End If
	End If
End Sub
		...
		If startupType <> "firstrun" Then
			...
			Do
				...
				If uName = "" Then
					Warn lang(521)
					Exit Sub
				Elseif fExists(dataLoc & "\User\" & uName & ".ebacmd") Then
					pWord = inputbox(strParam1(lang(009),lang(012)),title)
					If pWord = read(dataLoc & "\User\" & uName & "-enc.ebacmd","n") Then
						Exit Do
					Else
						Alert lang(217)
					End If
...
Sub migrate
	...
	If foldExists(dataLoc & "\Settings") Then
		...
		blockMalware = 2
		updateSet logging, saveLogin, defaultShutdown, shutdownTimer, blockMalware
	End If
...
Sub setLanguage
	...
	If Not fExists(dataLoc & "\PluginData\Lang\" & language & ".xml") Then
		...
		updateInf Edition, user, userPerms, ver, verID, language
	End If
...
End Function
Function updateInf(strA,strB,strC,strD,strE,strF)
	If Not foldExists(dataLoc) Then Exit Function
	write dataLoc & "\info.xml","<Info>" & tab(1) & "<Edition>" & strA & "</Edition>" & tab(1) & "<User>" & strB & "</User>" & tab(1) & "<Perms>" & strC & "</Perms>" & tab(1) & "<Version>" & strD & "</Version>" & tab(1) & "<VersionID>" & strE & "</VersionID>" & tab(1) & "<Language>" & strF & "</Language>" & vbcrlf & "</Info>"
End Function
Function updateSet(strA,strB,strC,strD,strE)
	write dataLoc & "\settings.xml","<Setting>" & tab(1) & "<Logs>" & strA & "</Logs>" & tab(1) & "<SaveLogin>" & strB & "</SaveLogin>" & tab(1) & "<ShutdownMethod>" & strC & "</ShutdownMethod>" & tab(1) & "<ShutdownTimer>" & strD & "</ShutdownTimer>" & tab(1) & "<BlockMalware>" & strE & "</BlockMalware>" & vbcrlf & "</Setting>"
End Function
...
Function writeFiles(bool1,bool2)
	...
	If Not portable Then
		cmd.RegWrite regLoc & "\", "","REG_SZ"
		cmd.RegWrite regLoc &		"\enableOperationCompletedMenu",ifStr(enableEndOp,True,1,0),"REG_DWORD"
		cmd.RegWrite regLoc & "\enableLegacyOperationCompletedMenu",ifStr(legacyMode,True,1,0),"REG_DWORD"
		cmd.RegWrite regLoc & "\installDir",programLoc,"REG_SZ"
		cmd.RegWrite regLoc & "\createRestorePointOnUpdate",ifStr(createRestore,True,1,0),"REG_DWORD"
		cmd.RegWrite regLoc & "\createRestorePointOnLaunch", ifstr(createSRPOnStart,True,1,0),"REG_DWORD"
		cmd.RegWrite regLoc & "\alwaysRunAsAdmin",ifstr(alwaysRunAdmin,True,1,0),"REG_DWORD"
	End If
	...
	If Not bool1 Then updateInf Edition, user, userPerms, ver, verID, language
