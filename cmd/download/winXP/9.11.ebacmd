'EBA Command Center for Windows XP
'Version ID 0911000-XP
'Update 9/13/21
...
Sub modeFirstrun
	...
	msgbox "Welcome to EBA Command Center!" & line & "We're going to perform initial setup now." & vblf & "If this is your first time using EBA Command Center, we recommend taking a look at our wiki!", 64, title
	eba = msgbox("Do you want to learn more about EBA Command Center by visiting the wiki?", 4+32+256, title)
	If eba = vbYes Then openWebpage("https://eba-cmd.fandom.com")
	eba = msgbox("If you have previously used EBA Command Center, and have a backup, you can restore that backup now, and skip initial setup! Restore a backup?", 4+32+256, title)
	If eba = vbYes Then
		eba = Replace(inputbox("Where is your backup file?",title),"""","")
		If fExists(eba) And LCase(Right(eba, 10)) = ".ebabackup" Then
			...
			If temp(0) = "exe.eba.restoreDuringSetup" or Not temp(0) Then
				Error "Looks like the restore failed! We'll continue with the initial setup.", "UnknownError"
			Else
				Note "Backup restored! Click OK to launch EBA Command Center."
				prog = 3
			End If
		Else
			Error "Either that file does not exist, or it is not a backup file. We'll continue intital setup.", "FileNotFoundOrFileNotBackup"
		End If
	End If
	...
	If prog <> 3 Then Call log("Critical Alert | New Admin Account created: " & uName)
	Call write(dataLoc & "\startupType.ebacmd","normal")
...
Sub modeNormal
	...
	If ver < curVer Then
		Call addDetails("nt")
		eba = msgbox("Notice:" & line & "An update for EBA Command Center is available! Do you want to update now?",4+64,title)
		If eba = vbYes Then
			If win7 = False And winXP = False Then
					...
					Else
						...
						Else
							...
							Else
								...
								End If
								If winXP = False Then
									eba = msgbox("Run this command with administrative privileges?",4+32+256,title)
									If eba = vbYes Then
										temp(5) = True
									Else
										temp(5) = False
...
Sub modeUpdate
	...
	delete(dataLoc & "\secureShutdown.ebacmd")
	delete(dataLoc & "\connect.ebacmd")
	delete(dataLoc & "\tmp")
	delete(dataLoc & "\.ebacrashreport")
...
Sub checkImports
	If LCase(Right(importData, 10)) = ".ebaimport" Or LCase(Right(importData, 10)) = ".ebabackup" Or LCase(Right(importData, 10)) = ".ebaplugin" Then
		...
		Elseif LCase(Right(importData, 10)) = ".ebaplugin" Then
			...
			If eba = vbYes Then
				checkWS
				fs.CopyFile importData, dataLocTmp & "\temp.zip"
				importData = dataLocTmp & "\temp.zip"
...
Sub checkPCName
	If win7 = False and winXP = False Then
		...
		If InStr(data,"<name>" & LCase(pcName) & "</name>") Then
			...
			critical "You've been banned!" & line & "This device has been banned from running EBA Command Center due to a violation of the Terms of Server. If this is an error, please contact EBA Software to have your PC removed from the blacklist. Make sure to send a copy of the error code below:","PC_Blacklist_" & pcName
			Call endOp("s")
		Elseif InStr(data,"<user>" & LCase(tmp) & "</user>") Then
			...
			critical "You've been banned!" & line & "This device has been banned from running EBA Command Center due to a violation of the Terms of Server. If this is an error, please contact EBA Software to have your PC removed from the blacklist. Make sure to send a copy of the error code below:","User_Blacklist_" & tmp
			Call endOp("s")
		End If
	End If
End Sub
...
Sub endOp(str)
	...
	delete dataLocTmp
	delete dataLocTmp & ""
	delete dataLoc & "\meta.ebatmp"
	delete dataLoc & "\temp.zip"
	dbm "Deleted EBA.Tmp."
